# LINE絵文字40個セットの作り方 完全ガイド（Instagramリール特典ページ）
...
- **2ルート構成**: Codexルート／ChatGPTのみルート、いずれも「キャラ提案→マスター画像生成→
  40個を1枚のグリッド画像として一括生成→切り出し・レビュー・ZIP作成」という同じ流れ
- **LINE公式仕様の厳守**: PNG（静止画）・180×180px固定・1個あたり1MB以下・背景透過必須
...
- **静止画40個方式を採用した経緯**: 当初は「動く絵文字（8個・APNG）」だったが、ChatGPTの画像生成が
  複数個のポーズを混同する・直前の生成を引きずる等の不具合が実機検証で繰り返し発生したため、
  信頼性を優先して静止画40個方式（競合LPと同方式）に全面刷新した。動く絵文字への再挑戦は
  現時点では推奨しない（`docs/SCOPE_PROGRESS.md`参照）。
