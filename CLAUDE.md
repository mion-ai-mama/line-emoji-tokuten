...
- **2ルート構成（作られるものが違う）**: Codexルート＝**動く絵文字（APNG）**／ChatGPTルート＝
  **静止絵文字（PNG）**。両ルートとも「キャラ提案→マスター画像生成→40個を1個ずつ個別生成→
  切り出し・レビュー・ZIP作成」という同じ流れだが、構成JSON・パイプラインスクリプトは別物
  （Codex＝`40-emoji-animated-plan.json`＋`emoji_pipeline_animated.py`＋`build_apng.py`／
  ChatGPT＝`40-emoji-set-plan.json`＋`emoji_pipeline.py`）
- **LINE公式仕様の厳守**:
  - 動く絵文字（Codex）：APNG・180×180px固定・10フレーム×100ms＝ちょうど1秒ループ×3回・
    1個あたり300KB以下・背景透過必須。**1ループの秒数が半端（1.2秒等）だと審査で弾かれる**ため、
    フレーム数・表示時間・ループ回数の組み合わせは必ず整数秒になるようにする
  - 静止絵文字（ChatGPT）：PNG・180×180px固定・1個あたり1MB以下・背景透過必須
  - 共通：セット8〜40個・メイン画像4個選出・タブ画像96×74px・提出ZIP20MB以内（`docs/requirements.md` §2.1）
- **目・口の絶対条件（動く絵文字のみ）**: 目は各フレームで「完全に開く」か「完全に閉じる」の
...
- **方式転換の経緯**: 当初「動く絵文字8個（APNG）」→ChatGPTの画像生成が不安定だったため
  「静止画40個」に全面転換→**競合の実際のLP（一次情報未確認だったため見落としていた）**を
  確認したところ、競合のメインルート（Codex）は動く絵文字だと判明。競合のCodex用プロンプトの
  検証ルール（目・口の完全開閉2状態、ループは整数秒のみ）を取り入れ、**Codexルートを動く絵文字に
  作り直した**。ChatGPTルートは信頼性を優先し静止絵文字のまま維持（`docs/SCOPE_PROGRESS.md`参照）。
